...
InAppPurchaseManager.prototype.makePurchase = function(productId, quantity) {
    ...
    return cordova.exec(function(){
                        alert('made purchase: '+arguments);
                        },function(ex){
                        alert('make purchase err: '+ex);
                        },'InAppPurchaseManager','makePurchase', [productId, q]);
}
...
InAppPurchaseManager.prototype.restoreCompletedTransactions = function() {
    return cordova.exec(function(){
                        alert('restore completed success: '+arguments);
                        },function(ex){
                        alert('restore completed  err: '+ex);
...
InAppPurchaseManager.prototype.requestProductData = function(productId, successCallback, failCallback) {
    ...
    success: function(productId, title, description, price ) {
        //alert(productId+'-'+title+'-'+description+'-'+price);
        if (productId == '__DONE') {
    ...
    cordova.exec(function(s1,s2,s3,s4){
                 //alert('request success: '+s1+','+s2+','+s3+','+s4);
                 },function(ex){
                 alert('request err: '+ex);
                 },'InAppPurchaseManager','requestProductData', [productId,callback + '.success', callback + '.fail']);
}
...
cordova.addConstructor(function()  {
                       ...
                       $(document).trigger('inappPurchaseLoaded');
                       });