...
TolitoConstructor.prototype = {
    ...
    init: function () {
        ...
        if (this._isRunning) {
            ...
            obj.fillProgressBar = setInterval((function (inst) {
                return function () {
                    var thisValue = 0;
                    try {
                        thisValue = $(inst.getId()).progressbar('option', 'value') * 1;
                    } catch(ex) {
                        console.log('get progressbar value error: ' + ex + ', id: ' + inst.getId());
                    }
                    var counter = thisValue > 0 ? (thisValue + 1) : 1;
                    if (counter >= inst.getMax()) {
                        clearInterval(obj.fillProgressBar);
                    } else {
                        $(inst.getId())
                            .progressbar({
                            value: counter
